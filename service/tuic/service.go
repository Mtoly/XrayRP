...
func (s *TuicService) failWithRuntimeOwnership(primary error, clientInfo api.ClientInfo, nodeInfo *api.NodeInfo, runtime runtimeInstance, inboundTag, tag string, startAt time.Time, tasks *specialruntime.Tasks) error {
	s.lifecycleMu.Lock()
...
func (s *TuicService) StartContext(parent context.Context) (err error) {
...
func (s *TuicService) CloseContext(parent context.Context) error {
	...
	if s.state == stateStarting || s.state == stateStopping {
		s.lifecycleMu.Unlock()
		return errors.New("TUIC service cannot close while starting or stopping")
	}
	tasks := s.tasks
	s.lifecycleMu.Unlock()
	...
	if s.state == stateStarting || s.state == stateReloading || s.state == stateStopping {
		...
		s.lifecycleMu.Unlock()
		s.reloadMu.Unlock()
		return errors.Join(producerStopErr, syncWaitErr, fmt.Errorf("TUIC service cannot close from state %d", state))
	}
...
func (s *TuicService) reloadNodeContext(parent context.Context, nodeInfo *api.NodeInfo) error {
	if nodeInfo == nil {
		return nil
	}
...
func (s *TuicService) reloadNodeLockedContext(ctx context.Context, nodeInfo *api.NodeInfo) error {
...
func (s *TuicService) reloadNodeWithCertificateLockedContext(ctx context.Context, nodeInfo *api.NodeInfo, renewal preparedCertificateRenewal) (err error) {
	if renewal != nil {
		defer func() {
...
func (s *TuicService) finishReload(runtime runtimeInstance, nodeInfo *api.NodeInfo, tag, inboundTag string, certConfig *mylego.CertConfig, state lifecycleState, runtimeErr error) {
	s.finishReloadWithOwnership(runtime, nil, nodeInfo, tag, inboundTag, certConfig, state, runtimeErr)
}

func (s *TuicService) finishReloadWithOwnership(runtime runtimeInstance, cleanupRuntimes []runtimeInstance, nodeInfo *api.NodeInfo, _, _ string, certConfig *mylego.CertConfig, state lifecycleState, runtimeErr error) {
	s.lifecycleMu.Lock()
	s.mu.Lock()
