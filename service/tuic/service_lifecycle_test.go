...
func TestRuntimeLifecycleSeamCompiles(t *testing.T) {
	...
	var factory runtimeFactory = func(*TuicService, runtimeBuildSpec) (runtimeInstance, string, error) {
...
func TestBuildRuntimeRecordsBuildAndReturnsFactoryError(t *testing.T) {
	...
	events := &lifecycleEvents{}
	service := &TuicService{
		runtimeFactory: func(*TuicService, runtimeBuildSpec) (runtimeInstance, string, error) {
			events.add("build")
			return nil, "", wantErr
		},
	}
...
func TestNewInstallsDefaultsWithoutSharingFactoryState(t *testing.T) {
	...
	first.runtimeFactory = func(*TuicService, runtimeBuildSpec) (runtimeInstance, string, error) {
...
func TestStartBuildsRuntimeWithIncomingTag(t *testing.T) {
	...
	service.runtimeFactory = func(_ *TuicService, spec runtimeBuildSpec) (runtimeInstance, string, error) {
		events.add("build")
		...
		if spec.inboundTag != "Tuic_127.0.0.1_8443_8" {
...
func TestStartBuildKeepsCandidateStatePrivate(t *testing.T) {
	events := &lifecycleEvents{}
	service := newStartTestService(events, &fakeRuntimeInstance{events: events})
	oldNode := &api.NodeInfo{NodeType: "Tuic", NodeID: 99, Port: 9443, TuicConfig: &api.TuicConfig{}}
	...
	service.runtimeFactory = func(got *TuicService, spec runtimeBuildSpec) (runtimeInstance, string, error) {
		...
		panelNode := service.apiClient.(*configurablePanelClient).nodeInfo
		if spec.nodeInfo == panelNode || !reflect.DeepEqual(spec.nodeInfo, panelNode) ||
			spec.inboundTag != "Tuic_127.0.0.1_8443_8" ||
	...
	if service.nodeInfo == oldNode || service.tag != "Tuic_127.0.0.1_8443_8" {
...
func TestStartBuildSpecMutationCannotChangePanelOrAppliedState(t *testing.T) {
	...
	client.nodeInfo.TuicConfig.ALPN = []string{"h3"}
	...
	if client.nodeInfo.Port != 8443 || client.nodeInfo.TuicConfig.ALPN[0] != "h3" {
		t.Fatalf("runtime factory mutated panel node: %+v", client.nodeInfo)
	}
	if service.nodeInfo == client.nodeInfo || service.nodeInfo.Port != 8443 ||
