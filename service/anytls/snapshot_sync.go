...
func (s *AnyTLSService) initializeSnapshotSyncCoordinator() {
...
func (s *AnyTLSService) SubmitSnapshotSync(trigger service.SnapshotSyncTrigger) {
...
func (s *AnyTLSService) syncSnapshotsContext(ctx context.Context, scope service.SnapshotSyncScope) error {
	var errs []error
...
func (s *AnyTLSService) syncUserSnapshotContext(ctx context.Context) error {
	users, err := api.GetUserListContext(ctx, s.apiClient)
	if errors.Is(err, api.ErrUserNotModified) {
		return nil
	}
	if err != nil {
		return fmt.Errorf("refresh AnyTLS users: %w", err)
	}
...
func (s *AnyTLSService) syncRuleSnapshotContext(ctx context.Context) error {
	...
	if err != nil {
		return fmt.Errorf("refresh AnyTLS rules: %w", err)
	}
	if rules == nil {
		return nil
	}
...
func (s *AnyTLSService) syncNodeSnapshotContext(ctx context.Context) error {
	...
	if err != nil {
		return fmt.Errorf("refresh AnyTLS node: %w", err)
	}
	if err := ctx.Err(); err != nil {
		return err
	}
	if nodeInfo == nil || nodeInfo.NodeType != "AnyTLS" {
		return errors.New("refresh AnyTLS node returned unexpected node type")
	}
...
func (s *AnyTLSService) recordSnapshotSyncResult(result specialruntime.SnapshotSyncResult) {
	if result.Err == nil {
	...
	if s.logger != nil {
		s.logger.Warn("AnyTLS snapshot sync failed; polling will retry")
	}
}
