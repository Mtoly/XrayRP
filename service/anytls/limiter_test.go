...
func TestRoutedConnectionAttachesConfiguredLimiter(t *testing.T) {
	...
	if !ok {
		t.Fatalf("RoutedConnection() = %T, want *connCounter", tracked)
	}
	defer counter.Close()
...
func TestRoutedPacketConnectionAttachesConfiguredLimiter(t *testing.T) {
	...
	tracked := (&anyTLSTracker{svc: service}).RoutedPacketConnection(
		context.Background(),
		connection,
		adapter.InboundContext{User: "user"},
		nil,
...
func limiterTestService() *AnyTLSService {
	service := New(&configurablePanelClient{}, &controller.Config{})
...
func assertLimiterCloseCleanup(t *testing.T, service *AnyTLSService, host string) {
	t.Helper()
	service.mu.RLock()
